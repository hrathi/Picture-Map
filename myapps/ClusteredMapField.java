...
public class ClusteredMapField extends MapField {
	...
	// crux of the algorithm to display one icon for a cluster.
	// it works as follows: for a given mappable item (GsImage),
	// if its rectangular bounding box intersects with another "Cluster"
	// then add it to that cluster. If it doesn't intersect with any other
	// cluster, create a new cluster and mark this item as "Lead" of the
	// new cluster.
	...
	private void setOuterBox(GsImage g) {
		...
		/*
		 * MapPoint[] pts = new MapPoint[5]; pts[0] = new
		 * MapPoint(gRect.getTop(), gRect.getLeft()); pts[1] = new
		 * MapPoint(gRect.getTop(), gRect.getRight()); pts[2] = new
		 * MapPoint(gRect.getBottom(), gRect.getRight()); pts[3] = new
		 * MapPoint(gRect.getBottom(), gRect.getLeft()); pts[4] = new
		 * MapPoint(gRect.getTop(), gRect.getLeft());
		 * 
		 * MapPolyLine line = new MapPolyLine(pts) { public boolean
		 * isClusterable() { return false; } };
		 * 
