            ...
            {           
				...
				if( _coordinates.containsKey(fileName) ) {
					...
					if( newCoord instanceof PersitableCoordinates ) {
						GeoImage gsImg = new GeoImage(fileName, ((PersitableCoordinates)newCoord).getLat(), ((PersitableCoordinates)newCoord).getLon(), fileName);
						_imgVector.addElement(gsImg);	
					}
				} else {
					Coordinates newCoord = getJPEGCoordinates(fileName);
					if( newCoord instanceof Coordinates ) {
						GeoImage gsImg = new GeoImage(fileName, newCoord.getLatitude(), newCoord.getLongitude(), fileName);
						_imgVector.addElement(gsImg);	
						_coordinates.put(fileName, new PersitableCoordinates(newCoord.getLatitude(), newCoord.getLongitude()));
					} else {
    		...
    		while (e.hasMoreElements()) {
    			...
    			if (root.equalsIgnoreCase("sdcard/")) {
    				try {
    					...
    					fc = (FileConnection) Connector.open(prefixPath,
    							Connector.READ);
    					if (fc != null) {
