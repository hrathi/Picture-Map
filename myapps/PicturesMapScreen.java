...
class PicturesMapScreen extends FullScreen implements FieldChangeListener {
	...
	public PicturesMapScreen() {
		...
		_mapField.getAction().disableOperationMode(
		...
		UiApplication.getUiApplication().invokeLater(new Runnable() {
			public void run() {
				UiApplication.getUiApplication().pushScreen(
						new ProgressIndicatorScreen(PicturesMapScreen.this));
			}
		});
	}
			...
			if (field == _mapField) {
				...
				if (m instanceof ClusterMarkers) {
					...
					if (superClusterDown) {
						...
						UiApplication.getUiApplication().pushScreen(
								new PictureViewerScreen(_upMarker._markers));
		...
		if (_upMarker != null && !_upMarker.isSuperCluster()) {
			UiApplication.getUiApplication().invokeLater(new Runnable() {
				...
				public void run() {
					if (_upMarker != null) {
						String uri = _upMarker.getIconUri();
						_upMarker.setIconUri("res://img/red"
	...
	public static Bitmap getScaledBitmapImage(String imagePath,
		...
		try {
			...
			if (connection.exists()) {
				...
				Bitmap b = Bitmap.createBitmapFromBytes(byteArray, 0, -1,
						scaleFactor);
				b.scaleInto(bitmap, Bitmap.FILTER_BILINEAR,
